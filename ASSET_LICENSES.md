...
- 文件：`cities/xuzhou/map.png`
- 类型：AI 生成内容，经项目作者选择并用于抽象游戏地图
- 生成工具：OpenAI 图像生成工具
- 生成日期：2026-08-13
- 用途：徐州版本的总览与局部场景底图
- 使用范围：徐州景点的非官方、非精确地理表达
- 许可：[Creative Commons Attribution 4.0 International（CC BY 4.0）](https://creativecommons.org/licenses/by/4.0/)
- 建议署名：`Xuzhou pixel map for Pixel City Walk, generated with OpenAI image generation, CC BY 4.0`

地图按照项目确认的相对方位制作；道路、水系、建筑比例和地标外观经过艺术化处理，不应用于现实导航，不得暗示徐州市政府、景区或 OpenAI 对本项目提供官方背书。

## 徐州旅行票根明信片

- 文件：`cities/xuzhou/ticket-base.png`
- 类型：AI 生成的徐州旅行纪念票根插画
- 生成工具：OpenAI 图像生成工具
- 生成日期：2026-08-13
- 用途：玩家结束徐州漫游后的固定下载底图
- 使用范围：徐州景点的非官方、艺术化相对方位表达
- 许可：[Creative Commons Attribution 4.0 International（CC BY 4.0）](https://creativecommons.org/licenses/by/4.0/)
- 建议署名：`Xuzhou travel ticket artwork for Pixel City Walk, generated with OpenAI image generation, CC BY 4.0`

底图没有画入昵称、编号、日期、路线或统计文字；这些内容由游戏引擎按 `cities/xuzhou/city.json` 的 `postcard.overlay` 配置在运行时动态绘制。底图中的景点比例和道路均为艺术化表达，不用于现实导航。
