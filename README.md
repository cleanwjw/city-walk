...
[![Default city](https://img.shields.io/badge/默认城市-徐州-a84b3f.svg)](#徐州版本彭城漫游记)
...
4. 收集七枚区域印章，完成主题路线或“古今三角主线”；
5. 结束漫游，生成并下载对应结局的旅行明信片。

## 徐州版本：彭城漫游记

仓库默认加载徐州版本，同时保留济南示例。玩家可以在抽象的徐州像素世界中选择出发地点、控制人物游览、查看景点介绍、记录打卡路线，并生成带昵称的旅行纪念卡。

![彭城漫游记示例地图](./cities/xuzhou/map.png)

## 当前功能

- 徐州像素世界总览
- 键盘、鼠标与触屏移动
- 回龙窝、户部山、徐州博物馆、汉文化景区、云龙湖、云龙山与淮海战役烈士纪念塔等区域
- NPC 新手引导
- 三条可随时切换的主题路线：山水漫游、老城寻踪、汉风与记忆
- “徐州博物馆—云龙山—淮海纪念塔”古今三角主线
- 七枚区域旅行印章与彭彭动态进度对话
- 景点介绍与旅行手账
- 本地浏览器进度保存（昵称、主题路线、手账、印章和解锁进度自动保存）
- 四级结局明信片与 PNG 下载：彭城初见、主题路线、古今三角、七印圆满
- 桌面端和移动端适配
...
│       └── LICENSE.txt
├── tests/
│   └── innovation-smoke.js   # 桌面、手机、PNG 与旧城市兼容流程测试
├── ASSET_LICENSES.md
...
本项目不是徐州或济南市政府、文化和旅游部门、景区或地图服务商的官方产品，也不代表任何官方推荐路线。像素地图属于抽象艺术表达，不应用于导航。景点信息可能随时间变化，实际游览请以相关机构的最新官方信息为准。
